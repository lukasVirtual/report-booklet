            ...
            <v-dialog v-model="dialog" width="600px">
                ...
                <v-card width="550" height="450" rounded>
                    ...
                    <v-card-actions>
                        ...
                        <v-spacer></v-spacer>
                        <v-btn color="blue" text @click="addNewUser">
                            Register
                        </v-btn>
                    </v-card-actions>
                </v-card>
            ...
            <v-dialog v-model="relationDialog" width="600px">
                <template v-slot:activator="{ attrs }">
                    <v-btn
                        ...
                        "
                        ><v-icon
                            >mdi-relation-one-or-many-to-only-one</v-icon
                        ></v-btn
                    >
                </template>
                ...
                <v-card width="550" height="450" rounded>
                    ...
                    <v-card-actions>
                        ...
                        <v-spacer></v-spacer>
                        <v-btn color="blue" text @click="assignUser">
                            Assign
                        </v-btn>
                    </v-card-actions>
                </v-card>
    ...
    setup() {
        ...
        const loadData = async (): Promise<void> => {
            ...
            let getInstructors = data.value.filter(
                (v: { Role: string }) =>
                    v.Role === "instructor" || v.Role === "user"
            );
            for (const p of getInstructors) {
                if (
                    p.Role === "instructor" &&
                    !instructors.value.includes(p.name)
                )
        ...
        onMounted(async () => {
            role.value =
                (await loginService.getUserRole()) as unknown as string;
            data.value = await dataService.getAllData();
